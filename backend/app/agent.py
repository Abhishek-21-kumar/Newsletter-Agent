...
def write_node(state: AgentState) -> Dict[str, Any]:
    ...
    return {
        ...
        "logs": logs,
        "status": "reviewing",
        # Clear user feedback once addressed
        "user_feedback": ""
    }
...
def review_node(state: AgentState) -> Dict[str, Any]:
    ...
    # We skip critique if we already did 2 self-revisions to avoid loop bloat
    if revision_count >= 2:
    ...
    if critique_status == "REVISE" and critique:
        ...
        return {
            ...
            "logs": logs,
            "status": "writing"  # Will loop back to write node
        }
    else:
...
def hitl_node(state: AgentState) -> Dict[str, Any]:
    # This node is hit when we resume from the HITL interrupt.
    # It logs user choice and determines the next step.
...
def send_node(state: AgentState) -> Dict[str, Any]:
    ...
    saved_msg = f"Newsletter successfully saved to file: output/{html_filename}"
...
def review_routing(state: AgentState):
    """
    Decides whether to route back to write step (if critique requests rewrite) or proceed to hitl block.
    """
    # If the status is 'writing', it means review_node flagged for rewrite and incremented revision
...
def hitl_routing(state: AgentState):
    """
    Routes from HITL node.
    If the status is 'writing', we loop back to write step.
    If approved or completed, we go to send step.
    """
    if state.get("status") == "writing":
        return "write"
    return "send"
...
# We set interrupt before the HITL node. If the execution is HITL, it pauses.
memory = MemorySaver()
